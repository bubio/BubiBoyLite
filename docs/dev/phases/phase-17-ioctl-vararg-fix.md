...
2026-07-19 T17-3 完了: pty検証(race-freeなwinsize設定: フォークより前に
`fcntl.ioctl(master, TIOCSWINSZ, ...)` を適用し、子プロセスの最初のフレームから
既に最終サイズが見えるようにした上で、実VT100/xterm系エミュレータ `pyte` で
バイトストリームを解釈)を80x24・119x41・119x44・100x30の4サイズで、ホーム画面/
ゲーム中画面の両方に対して実施。全サイズ・両画面で「コンテンツが実際に画面最終行
(rows行目)まで届く」「区切り線がrows-2行目、ステータス行(fps含む)がrows-1行目、
入力行がrows行目にちょうど来る」ことを直接確認した(3回連続実行で再現性も確認)。
これはT16-3までの構造的検証(区切り線・入力行の"存在"のみ確認)と異なり、**正確な
行位置そのもの**を検証した初めてのテストであり、T17-1の修正(ioctlのABIバグ修正)
により実際に「フォールバック値ではなく実際の端末サイズどおりに」固定レイアウトが
機能するようになったことを直接裏付けた。

**検証中に発見した経緯(記録として残す)**: 検証スクリプトの初期実装では
`pyte.Stream.feed()` をリアルタイムの読み取りループ内で逐次呼び出していたところ、
100x30サイズの4番目のテストでのみ、区切り線の行に古いステータス文字列が
不自然に混在する現象が再現性高く(2/2)発生した。生バイト列を全て一括取得してから
`repr()` で直接確認したところ、**アプリ自身が出力する実際のバイト列には異常が
無い**ことを確認(区切り線・ステータス行・入力行は改行で正しく分離されていた)。
これは `pyte` の逐次 `feed()`呼び出しとリアルタイム読み取りのタイミングが絡み合う
ことで生じるテストハーネス側だけの見かけ上のアーティファクトと判断し、全バイトを
バッファしてから一括で `feed()` する方式に検証スクリプトを修正したところ、
3回連続で全項目パスした(アプリ自体の出力に問題は無いことを再確認)。

2026-07-19 T17-4 完了(自動検証は完了・クラッシュなし。実機確認のみ残、チェックは
付けない): `odin test tests -collection:bbl=src` 474件全パス(-o:speed/-debug両方)、
`./scripts/build_macos.sh --debug --test` 成功。pty回帰確認一式を、ioctl修正が
実際に効いた状態(初めて実際のwinsize検出パスが動く状態)で再実行し全パス:
(1) T14固定レイアウト回帰確認19項目、(2) T13/T14フルラウンドトリップ3回、
(3) T15総合コマンドオンリー確認14項目、(4) T16-1のNow Playing簡素化確認8項目、
(5) T17-3の80x24/119x41/119x44/100x30の4サイズ×ホーム/ゲーム両画面の正確な行位置
確認(20項目)、(6) T14-6のリサイズ+SIGTERM復元確認5項目。全てクラッシュ・
リグレッションなし。`-debug` ビルドでも100x30での行位置確認がクリーン。
各検証ともbbl.iniは検証前後でバイト一致。最終バイナリは `-o:speed` で再ビルド済み。
**残項目**: 実機 macOS Terminal.app(Apple Silicon Mac、119x44等)での最終確認 —
実際にコマンドエリアが画面最下部に来るようになったこと。今回の修正はFableが実機で
発見・特定した根本原因(ioctlのFFI宣言バグ)に対する直接修正であり、本開発環境の
pty検証(ioctlの成功/失敗を直接計測)でも修正前後の違いを明確に再現・確認できたため
高い確度で解消していると考えられるが、最終確認はユーザー側にお願いする。
PLAN.mdはこのため🟡のまま、タスク数3/4で据え置く。
