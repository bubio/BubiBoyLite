...
2026-07-11 T4-7 完了(フェーズ4マイルストーン): scripts/fetch_test_roms.sh に
emulator-only/mbc1(12本)・mbc2(7本)・mbc5(8本)の fetch_mooneye 呼び出しを追加
(mbc1/multicart_rom_8Mb.gbはMBC1M専用でスコープ外のため取得しない、コメントで明記)。
`sh scripts/fetch_test_roms.sh` を実行し既存キャッシュから全27本が揃っていることを確認
(冪等性も確認: 既存ファイルは「取得済み、スキップ」)。tests/mooneye_test.odin に
emulator-only/mbc1・mbc2・mbc5の@(test)を計27件追加(T4-2/T4-3/T4-5で個別に手動検証済み
だったROM群を正式なテストとして昇格)。expected_failures.odin の変更は無し
(rapid_toggle・oam_dma/reg_readはMBC無関係でT4範囲外、指示どおり深追いしていない)。
mbc1/multicart系は「取得しない」方針のため許可リストへの追加も不要(テスト自体が存在しない)。
cpu_instrs.gb統合版(T4-2で追加済み)は今回のフルスイート実行でも継続PASS。
`odin test tests -collection:bbl=src` 225 tests 全パス(既存198 + 新規27)、17.3秒、
mbc1/mbc2/mbc5の27本含め全てPASS。`odin build src/app -collection:bbl=src`もクリーン。

**フェーズ4完了条件の達成状況(正直な記録)**:
- cpu_instrs統合版PASS: 達成(自動テスト)。
- Mooneye MBC系(mbc1/mbc2/mbc5、multicart除く)PASS: 達成(自動テスト、27/27)。
- .sav が再起動後も残る: 達成(T4-6のcore単体テスト9件 + 自作ホームブリューROMでの
  結合確認。市販ROM未検証)。
- 「MBC1とMBC5の市販ゲーム各1本が起動しプレイ可能」(T4-7のDoD文言): **未検証**。
  CLAUDE.mdの方針どおり市販ROMは入手・同梱できないため、Mooneye自動テスト全パスと
