...
- [ ] ゲーム中に `s`/`l`/`p`/`+`/`-`/`1`-`4` を単独で押しても即時発火せず、入力行に文字が
      入るだけであること
- [ ] `/save`・`/load`・`/pause`・`/resume`・`/slot N`・`/volume up`・`/volume down`・`/set` が
      全て従来どおり機能すること
- [ ] 固定レイアウト(入力行・ステータス行が最下部固定、リサイズ追従)のリグレッションが
      無いこと
- [ ] `-o:speed` フルラウンドトリップ、`-debug` ビルドも成功
