...
- [ ] 完了(実機GUI確認が残っているため未完了、下記参照。-o:speed ビルド固有の重大バグを
      発見・修正済み、詳細は検証ログ参照)

**目的**: フェーズ12のマイルストーン。
**作るもの**: デバッグと修正のみ。チェックリスト:
- [x] `./bbl` → ロゴ+プロンプト → `/browse` → ROM ブラウザ → `q` → ホームへ戻る、が一巡
- [x] `--recent` でホームをスキップし直接 recent 画面
- [x] `/settings`/`/set` で `bbl.ini` に差分だけ反映(コメント・行順保持)
- [x] `--scale 6` 等 CLI 併用時、`/set` を使わなければ `bbl.ini` の scale が変化しない
- [x] ゲーム実行中に `/` でコマンドモード、SDL ウィンドウが応答不能にならない、`/set volume 50` が機能
      (pty経由でターミナル側の配線を確認。SDLウィンドウそのものの目視確認は次項参照)
- [ ] macOS Terminal / Linux 実機確認(この開発環境には対話的GUIセッションが無く実施不能、
      T9-6と同じ既知の制約。ユーザーによる実機確認が必要)
...
2026-07-17 T12-6 途中経過(未完了): チェックリスト6項目中5項目をpty(`pty.openpty`)経由で
確認・チェック済み。追加で「`--scale 6` 起動でも `/set` を使わなければ `bbl.ini` の scale が
デフォルト値のまま変化しない」ことを確認(CLI引数の一時的な上書きが誤って永続化されない、
プランの中核方針の一つ)。`odin test tests -collection:bbl=src` 447件全パスを再確認。
T12-3〜T12-5でそれぞれ実施した pty 検証シナリオ4本(ホーム画面遷移、--recentスキップ、
/settings・/set、ゲーム中コマンドモード)を再実行し、レグレッションが無いことを確認。
残る1項目(macOS Terminal / Linux 実機での目視確認、特にゲーム中コマンドモードは SDL
ウィンドウのフォーカス切り替えを伴うため重要度が高い)は、この開発環境に対話的GUI
セッションが無く実施不能(T9-6で踏んだのと同じ既知の制約)。次回、実際のターミナル操作が
可能なセッションでユーザーによる実機確認を行い、問題なければT12-6をチェックしてフェーズを
🟢に更新すること。PLAN.mdはこのため🟡のまま、タスク数を5/6として据え置く。

2026-07-18 T12-6 続報(重大バグ発見・修正、依然未完了): 上記の「pty検証で確認済み」は
`-debug` ビルドのみでの検証だった。CLAUDE.md「ビルドスクリプトのデフォルトは `-o:speed`」
を踏まえ `config_patch_ini`/`config_apply_set` の `context.temp_allocator` 使用箇所を
T9-6の教訓に基づき `context.allocator` へ修正した際に、初めて `-o:speed` での全体検証を
実施したところ、`-debug` では再現しない2件のクラッシュを新たに発見した。
**発見1(ホーム画面描画)**: `tui_write_home_screen` が `os.write_string(os.stdout, s)` を
直接呼ぶと、`-o:speed` ビルドのループ2周目で `runtime assertion: a.allocator.procedure != nil`
(`core_builtin.odin:1376`、`_reserve_dynamic_array` 内)が発生することを、デバッグマーカーに
よるバイセクションで特定。呼び出し直前は `context.allocator.procedure != nil` だが直後に
`nil` になる。同一パターンの既存 `tui_write_frame`(ROM ブラウザ側)は別の呼び出し元では
発火しない。"contextless" な `tui_plat_write_raw`(syscall を直接呼び context を経由しない、
既存の alt screen 制御で実績あり)に置き換えて解消(discriminating test で
`os.write_string`→5/5クラッシュ、`tui_plat_write_raw`→5/5クリーンを確認)。
**発見2(/set 確定直後)**: `tui_run_command_home` のループ内、`config_apply_set()` から
戻った直後にループ先頭へ戻り `tui_term_size()` を呼ぶと同種のアサーションが発生。
config.odin 側の各関数(`config_patch_ini`/`config_patch_file`/`config_apply_set`)を個別に
`@(optimization_mode="none")` にしても直らず、`os.read_entire_file` 直後の
`context = runtime.default_context()` 明示復元も効果なし。切り分けの結果、
`tui_run_command_home`・`tui_term_size`・`tui_plat_term_size`(tui_posix.odin/tui_windows.odin)
の3つ全てに `@(optimization_mode="none")` を付けて初めて解消することを確認した
(3点セット、いずれか1つを外すと再発する可能性があるため3つとも保持する設計とした)。
予防的に同型のループを持つ `tui_run_settings_menu` にも同じ属性を付与(実機検証では
発火しなかったが、同じ危険パターンを含むため)。一方 `run_rom_window`(main.odin、
ゲーム中 `/set`)は同じ `config_apply_set` を呼ぶがフレーム構造が異なり、pty検証5回で
クラッシュしなかったため属性は付けていない。
**検証手法上の副次的発見(AMFI)**: pty検証でバイナリを一時ディレクトリへ `shutil.copy`
してから実行する従来方式は、macOS の AMFI(コード署名検証)が非同期に
「no CMS blob / Unrecoverable CT signature」と判定し `SIGKILL`(signal 9)でプロセスを
強制終了させることがあると判明した(`log show` でカーネルログ確認)。これは今回の
`-o:speed` バグとは無関係な誤検知だったため、以降は実行ファイルをコピーせず
プロジェクトディレクトリのバイナリを絶対パスで直接 exec し、cwd のみ一時ディレクトリに
変更する方式(in-place 実行)に検証手法を切り替えた。この誤検知は署名なしバイナリを
配布 zip として展開・実行した場合にも起きうるため、phase-11(リリース配布)で留意すべき
既知事項として記録しておく(今回のスコープ外、対処は次回以降)。
**再検証**: `odin test tests -collection:bbl=src` 447件全パス(`-o:speed`)。in-place pty検証:
ホーム画面遷移(5回)、`--recent`スキップ(3回)、`/settings`+`/set`(ホーム画面、8回)、
`/settings` 対話メニュー内での実際の編集操作(3回、従来ワンライナーのみで対話編集は未検証
だった箇所)、ゲーム中 `/set`(ROM実行、5回)、いずれも `-o:speed` でクラッシュなし。
プランの検証項目のうち従来カバーしていなかった2点も追加確認: (1)
`test_config_patch_ini_replaces_only_target_key`(既存ユニットテスト)がコメント・行順保持を
既に検証済みと確認、(2) 起動時自動生成された `bbl.ini` が `/settings` を開いて Esc で
閉じただけでは1バイトも変化しないこと、`--scale 6` 起動で `/quit` しても `bbl.ini` の
scale がデフォルト(4)のままであることを新規スクリプトで確認(3回)。
**未確認のまま**: ゲーム中コマンドモードでの SDL ウィンドウの実際の応答性(プロセス生存と
fps表示継続は確認したが、これは「クラッシュしない」ことの代理証拠であり、複数秒に渡る
コマンド入力中も `sdl.PollEvent` がフォーカス外でも回り続けているという保証ではない。
ヘッドレス環境では証明不能)。macOS Terminal / Linux 実機での目視確認(T9-6と同じ既知の
制約)。次回、実機確認が可能なセッションで実施し、問題なければT12-6を完了としチェックする。
PLAN.mdはこのため🟡のまま、タスク数を5/6として据え置く。
