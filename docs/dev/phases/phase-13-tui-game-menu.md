...
- [ ] 完了(自動検証は全て完了・クラッシュなし。実機 GUI での目視確認のみ残、下記
      チェックリストと検証ログ参照。T12-6 と同じ扱いでチェックは付けない)

チェックリスト:
- [x] 幅/高さ変化時のオーバーレイ強制再描画(game_menu_overlay_draw が cols/rows 両方を監視)
- [x] `-o:speed` でホーム→/settings→/browse→ROM起動→ゲーム中 /settings→←→調整→Esc→
      /quit→ホーム→/quit の往復 5回、クラッシュ・アサーションなし
- [x] `-debug` ビルドでも同じ往復 1回クリーン
- [x] `odin test tests -collection:bbl=src` 全パス、両ビルド成功
- [x] lone-ESC 二段 read hardening は今回スコープ外として記録のみ(T12-5 落とし穴に記載の
      既知の弱点のまま)
- [ ] macOS Terminal / Linux 実機での目視確認(SDL ウィンドウの実応答性・フォーカス切替。
      この開発環境には対話的GUIセッションが無く実施不能、T9-6/T12-6 と同じ既知の制約。
      ユーザーによる実機確認が必要)
...
2026-07-18 T13-6 途中経過(自動検証完了・実機GUI確認のみ残、チェックは付けない):
`game_menu_overlay_draw` を cols に加えて rows の変化でも強制再描画するよう拡張。
`-o:speed` リグレッション往復検証を pty(in-place 実行、単一ROMのみの一時ディレクトリを
cwd にして ROM ブラウザの選択を決定的にした)で実施: ホーム→`/settings`(scale `→`+`←`で
往復)→Esc→`/browse`→▸マーカー行を読んで ROM 選択→Enter 起動→ゲーム中 `/`→`settings`→
↓×3→`←`「volume = NN に更新しました」→`→`復元→Esc(`\x1b[0J`)→`/quit`→ホーム復帰→
`/quit` exit 0、を **5回連続クリーン**(T12-6 型アサーション再発なし。このため計画の段階的
対応(contextless 書き込み切替→opt-none 付与)は不要と判断し、`game_menu_overlay_draw` は
素の実装のまま。opt-none 3点セット+tui_run_settings_menu の既存属性は無変更で維持)。
`-debug` ビルドでも同じ往復 1回クリーン。`./scripts/build_macos.sh --debug --test` 成功、
`odin test tests -collection:bbl=src` 464件全パス、最終バイナリは `-o:speed` で再ビルド済み。
各検証とも bbl.ini は増減を対で行い検証前後でバイト一致(誤永続化なし)。
lone-ESC 二段 read hardening(オプション項目)は未実施のまま今回スコープ外として記録
(CSI シーケンスが read 境界で分割されると単独 ESC に誤判定されうる T12-5 記載の既知の弱点は
残存)。**残項目**: macOS Terminal / Linux 実機での目視確認(特にオーバーレイ表示中の SDL
ウィンドウの実応答性と、最下行スクロール時の表示崩れの有無)。実施できたら T13-6 をチェックし
フェーズを 🟢 に更新すること。PLAN.md はこのため 🟡 のまま、タスク数 5/6 で据え置く。
