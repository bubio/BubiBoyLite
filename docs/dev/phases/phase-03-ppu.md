...
2026-07-11 T3-8 完了(フェーズ3マイルストーン): `scripts/fetch_test_roms.sh`へのdmg-acid2取得追加は
T3-6で先行実施済み(依存関係のため)。`tests/acid2_test.odin`を新規作成: `emulator_load_rom`→
`emulator_run_frame`x100→`bus.ppu.framebuffer`のFNV-1a 64bitハッシュ(`core:hash.fnv64a`)を
期待値`0x17A0F9970AC4D084`と比較。期待値の決め方(testing.md準拠): 同一手順で生成した
framebufferを手書きBMPライター(24bit, ボトムアップ)で書き出し、macOSの`sips`でPNG変換して
Readツールで画像として確認。`img/reference-dmg.png`(公式reference)と目視比較し、顔の輪郭・
目の三日月装飾・鼻(ダイヤ形)・口・"HELLO WORLD!"/"dmg-acid2 by Matt Currie"のテキストが
全て一致(崩れなし)することを確認したうえでハッシュを固定した。

T3-8の過程で `mooneye/acceptance/halt_ime0_ei`・`halt_ime0_nointr_timing` を再調査し原因を特定・
解消した。トレースの結果、両ROMともLCDをROM側で明示的に有効化せず、DMGブートROM完了直後の
既定状態で既にLCDC=0x91(画面ON)であることを前提にVBlank割り込みを待つ作りだった。本プロジェクトは
実BIOSを読み込まない方針(CLAUDE.md)のため起動直後のレジスタ状態を直接セットする必要があるが、
T3-1時点ではCPUレジスタのみ実装しIOレジスタはゼロ初期化のままだった(LCDC=0x00=画面OFF)ため、
LYが一度も進まずVBlank割り込みが永遠に来ずHALTしたまま停止していた(トレース確認:
pc=0x0160でhalted=true、LCDC=0x00のまま12M T-cycle経過)。`ppu_power_on`(src/core/ppu.odin)・
`bus_power_on`(src/core/bus.odin)を追加し、DMG post-boot register state(Pan Docs "Power Up
Sequence"、BubiBoy Bus.fs postBootIoの実測値: LCDC=0x91, STAT=0x80, BGP=0xFC, OBP0=OBP1=0xFF)を
`emulator_load_rom`と`tests/rom_runner.odin`の両方から呼ぶようにした結果、両ROMともPASSに転じた
(`tests/expected_failures.odin`から除外)。この変更後もdmg-acid2のハッシュが変化しないことを
再テストで確認済み(自ROM内でLCDC等を明示的に再設定するため影響なし)。
`mooneye/acceptance/oam_dma/reg_read`は本変更後も引き続きFAILのため許可リストに残す
(disable_ppu_safe待ちは解消済みだが、DMA転送中のレジスタ読み出し値が期待と異なる。
フェーズ4以降で再調査)。`mooneye/acceptance/timer/rapid_toggle`はT2-7からの既知の保留事項
(ユーザー承認済み)のため今回も触れていない。

実ゲーム確認: 著作権上、市販ゲームROMは入手・使用していない(BluePrintの例示であり必須ではない、
セッション指示により明示的にホームブリュー/自作ROMへ置換)。T3-7で自作したROM-onlyホームブリュー
`input_demo.gb`(RGBDS 1.0.1で自作、d-padでスプライトを移動させる最小デモ、パブリックドメイン
相当)を用いて、SDL_PushEventによる合成キーイベント経由でinput.odinの実コードを実行させ、
スクリーンショットでスプライトの移動を確認済み(T3-7の検証ログ参照)。「タイトル→プレイまで動作する
市販/配布ゲーム1本」は用意できていないため、この項目はdmg-acid2(PPU描画の網羅的正しさの主証跡)と
input_demo.gb(入力の動作証跡)の組み合わせで代替したことを正直に記録する。

`odin test tests -collection:bbl=src` で全148本PASS(dmg_acid2含む)。
`odin build src/app -collection:bbl=src` ビルド成功。フェーズ3のマイルストーンを達成したため
PLAN.mdの状態を🟢へ更新する。
