...
2026-07-16 T11-4 完了: README.md を全面拡充（概要、対応プラットフォーム表、動作要件、インストール、
使い方（`cli.odin` の USAGE 全文 + ターミナル側ショートカット表）、`bbl.ini` 全項目リファレンス
（`config.odin` の Config 構造体・デフォルト値から転記）、ビルド方法、テスト ROM 入手、ライセンス節
（MIT + テスト ROM 非同梱の旨）、実 BIOS 非対応の明記）。
**スクリーンショット（TUI・ゲーム画面）は本環境に SDL2 ウィンドウを表示できるディスプレイが無く
撮影不可のため未収録**。README にはその旨と差し替え手順をコメントで明記した（プレースホルダ）。
